...
def bfs(root: node, goalNode: node, map: list):
    ...
    while len(queue) > 0:
        ...
        if cur_node.coordinates == goalNode.coordinates:
            ...
            depth = len(path) - 1
            return visited, path, totalCost, depth
        
        cur_node.generateSuccessorNodes(map)

        if cur_node.left is not None and cur_node.left.coordinates not in visited and cur_node.left.cost != 0:
            visited.append(cur_node.left.coordinates)
            queue.append(cur_node.left)

        if cur_node.right is not None and cur_node.right.coordinates not in visited and cur_node.right.cost != 0:
            visited.append(cur_node.right.coordinates)
            queue.append(cur_node.right)
            
        if cur_node.up is not None and cur_node.up.coordinates not in visited and cur_node.up.cost != 0:
            visited.append(cur_node.up.coordinates)
            queue.append(cur_node.up)

        if cur_node.down is not None and cur_node.down.coordinates not in visited and cur_node.down.cost != 0:
            visited.append(cur_node.down.coordinates)
            queue.append(cur_node.down)
        
def getPath(endNode, startNode):
    pathRev = []
    cur_node = endNode
    cost = 0
